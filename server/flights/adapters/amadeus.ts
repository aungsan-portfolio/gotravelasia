...
function diffMinutes(aIso?: string, bIso?: string): number {
  ...
  // Z-Hack to bypass NodeJS DST manipulation for calculating difference between two local times at the same physical target
...
function detectMixedCabin(travelerPricings?: any[]): boolean {
  ...
  const cabins = new Set(
    fareDetails.map((f: any) => String(f?.cabin || "").toUpperCase()).filter(Boolean)
  );
