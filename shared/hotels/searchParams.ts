...
const HOTEL_SORT_VALUES: HotelSort[] = [
  "best",
  "rank",
  "price_asc",
  "price_desc",
...
export function validateHotelSearchParams(
  ...
  if (
    ...
    Number.isNaN(params.adults) ||
    params.adults < 1 ||
    params.adults > MAX_ADULTS
  ) {
    errors.push(`Adults must be between 1 and ${MAX_ADULTS}.`);
  }
  if (
    typeof params.rooms !== "number" ||
    Number.isNaN(params.rooms) ||
    params.rooms < 1 ||
    params.rooms > MAX_ROOMS
  ) {
    errors.push(`Rooms must be between 1 and ${MAX_ROOMS}.`);
  }
  if (
    typeof params.page !== "number" ||
    Number.isNaN(params.page) ||
    params.page < 1 ||
  ...
  if (
    params.checkIn &&
    params.checkOut &&
    isIsoDate(params.checkIn) &&
    isIsoDate(params.checkOut)
  ) {
...
export function normalizeHotelSearchParams(
  input: Partial<HotelSearchParams> & { city?: string | null }
): HotelSearchParams {
  ...
  const normalized: HotelSearchParams = {
    ...
    sort: HOTEL_SORT_VALUES.includes(input.sort as HotelSort)
      ? (input.sort as HotelSort)
      : "best",
  };
...
function clampInteger(
  ...
  const parsed =
    typeof value === "number" ? value : parseOptionalInt(String(value ?? ""));
...
function sanitizeCityName(value?: string | null) {
  if (!value) return undefined;
